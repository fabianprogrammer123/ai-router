...
import { type NormalizedEmbeddingRequest } from '../types/request.js';
import { createAuthMiddleware } from '../middleware/auth.js';
import { requestIdMiddleware } from '../middleware/requestId.js';

export function createEmbeddingRoutes(router: Router, routerApiKey: string) {
  const authMiddleware = createAuthMiddleware(routerApiKey);

  return async function embeddingRoutes(fastify: FastifyInstance): Promise<void> {
    fastify.post(
      '/v1/embeddings',
      {
      ...
      async (request: FastifyRequest, reply: FastifyReply) => {
        const body = request.body as NormalizedEmbeddingRequest;
        ...
        const requestedModel = body.model ?? 'text-embedding-3-small';
